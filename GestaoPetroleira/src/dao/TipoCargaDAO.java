...
public class TipoCargaDAO {
    public List<TipoCarga> listarTodos() {
        ...
        try (Connection c = db.getConn(); Statement st = c.createStatement();
             ResultSet rs = st.executeQuery("SELECT id,nome,inflamavel,corrosiva,toxica FROM dias.TIPO_CARGA")) {
    ...
    public TipoCarga buscarPorId(int id) {
        try (Connection c = db.getConn();
             PreparedStatement st = c.prepareStatement("SELECT id,nome,inflamavel,corrosiva,toxica FROM dias.TIPO_CARGA WHERE id=?")) {
            st.setInt(1, id);
    ...
    public void eliminar(int id) {
        try (Connection c = db.getConn();
             PreparedStatement st = c.prepareStatement("DELETE FROM dias.TIPO_CARGA WHERE id=?")) {
